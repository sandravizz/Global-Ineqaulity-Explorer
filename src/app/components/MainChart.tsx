...
export function MainChart({ selectedGroups, setSelectedGroups, filters }: MainChartProps) {
  ...
  useEffect(() => {
    ...
    const chartData = Array.from(aggregatedData.values()).sort((a, b) => a.year - b.year);
    ...
    const transformedData = chartData.map((d, i) => {
      ...
      if (filters.yAxis === 'Income Share') {
        ...
        return {
          ...
          middle_40: ((d.middle_40 - baseData.middle_40) / baseData.middle_40) * 100,
          bottom_50: ((d.bottom_50 - baseData.bottom_50) / baseData.bottom_50) * 100,
  ...
  return (
    <div className="relative">
      ...
      <div className="flex gap-6 mb-4 text-sm">
        <button
          onClick={() => setSelectedGroups({ ...selectedGroups, top10: !selectedGroups.top10 })}
          className="flex items-center gap-2 hover:opacity-80"
        >
          <span className={`w-3 h-3 rounded-full ${selectedGroups.top10 ? 'bg-red-500' : 'bg-gray-600'}`}></span>
          Top 10%
        </button>
        <button
          onClick={() => setSelectedGroups({ ...selectedGroups, middle40: !selectedGroups.middle40 })}
          className="flex items-center gap-2 hover:opacity-80"
        >
          <span className={`w-3 h-3 rounded-full ${selectedGroups.middle40 ? 'bg-blue-500' : 'bg-gray-600'}`}></span>
          Middle 40%
        </button>
        <button
          onClick={() => setSelectedGroups({ ...selectedGroups, bottom50: !selectedGroups.bottom50 })}
          className="flex items-center gap-2 hover:opacity-80"
        >
          <span className={`w-3 h-3 rounded-full ${selectedGroups.bottom50 ? 'bg-lime-500' : 'bg-gray-600'}`}></span>
          Bottom 50%
        </button>
        <button
      ...
      {/* Chart */}
      <div ref={containerRef} className="bg-[#0a1628] rounded-lg p-4 border border-gray-700 relative w-full" style={{ maxWidth: '100%', overflow: 'hidden' }}>
        <svg ref={svgRef} style={{ display: 'block', maxWidth: '100%' }}></svg>
        
        {/* Tooltip */}
        {tooltipData && (
          <div
            className="absolute pointer-events-none bg-[#1e293b] border border-gray-600 rounded-lg p-3 shadow-xl"
            style={{
              ...
              <tbody>
                {tooltipData.values.map((row, i) => (
                  <tr key={i} className="border-b border-gray-700 last:border-0">
                    ...
                    <td className="text-right py-1.5 pr-4 text-white">{row.value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="flex gap-3 mt-4">
        <button className="flex items-center gap-2 px-4 py-2 bg-[#0d1f3a] border border-gray-600 rounded hover:bg-[#1a2f4a] text-sm">
          <Share2 className="w-4 h-4" />
          Share
        </button>
        <button className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded text-sm">
